...
// Tag deduplication using Jaccard word-set similarity.
// Iterates tags in order; skips any tag that is sufficiently similar to one already kept.
// threshold: 0.0–1.0 (default 0.5 → 50% word overlap triggers dedup)
function _tagWordSet(tag: string): Set<string> {
	return new Set(
		tag
			.toLowerCase()
			.trim()
...
export function deduplicateSemanticTags(tags: string[], threshold = 0.5): string[] {
	const result: string[] = []
	const resultSets: Set<string>[] = []
	for (const tag of tags) {
		if (!tag || typeof tag !== 'string') continue
		const ws = _tagWordSet(tag)
		const isDuplicate = resultSets.some(s => _jaccardSimilarity(s, ws) >= threshold)
		if (!isDuplicate) {
			result.push(tag)
			resultSets.push(ws)
		}
