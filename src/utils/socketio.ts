...
export const socketEvents = (socket: socketio.Socket, user: LeanDocument<UserType>) => {
  socket.handshake.query?.token &&
    jwt.verify(socket.handshake.query.token.toString(), config.secrets.jwt as string, async (err: any, decoded: any) => {
      if (err) {
        console.error('[Socket.io]: Authentication error')
        return
      }
      try {
        user = await getUser(decoded.id) as LeanDocument<UserType>
        if (!user) {
...
export const socketIO = async (server: any) => {
  try {
    ...
    io.on('connection', async (socket: socketio.Socket) => {
      ...
      socketEvents(socket, user as unknown as LeanDocument<UserType>)
    })
  } catch (err) {
    console.error(err)
